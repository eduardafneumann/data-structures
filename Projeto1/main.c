...
void ler_grafo(GRAFO *grafo)
{
    ...
    while (scanf("%d %d %d", &vertice1, &vertice2, &peso) != EOF)
    {
        vertice1--; vertice2--;
...
INFO *info_gerar()
{
    // lendo input
    int n_cidades;
    if(scanf("%d", &n_cidades) < 1) printf("Erro de leitura");
    int cidade_origem;
    if(scanf("%d", &cidade_origem) < 1) printf("Erro de leitura");
    cidade_origem--;
    ...
    for(int i = 0; i < n_cidades; i++) visitados[i] = false;
    ...
    info->grafo = grafo;
    info->cidade_origem = cidade_origem;
    info->visitados = visitados;
    info->melhor_solucao = melhor_solucao;
    info->caminho_atual = caminho_atual;
